...
bind Classy::Table::ylabel <FocusOut> {Classy::Tableobject %W _setyl %W;break}
bind Classy::Table::ylabel <<Return>> {Classy::Tableobject %W _setyl %W;break}
bind Classy::Table::xlabel <FocusOut> {Classy::Tableobject %W _setxl %W;break}
bind Classy::Table::xlabel <<Return>> {Classy::Tableobject %W _setxl %W;break}
# ------------------------------------------------------------------
#  Widget creation
# ------------------------------------------------------------------
...
#doc {Table options -setcommand} option {-setcommand setCommand Command} descr {
...
#doc {Table options -setxlabelcommand} option {-setxlabelcommand setXlabelCommand Command} descr {
# gives the command that will be called to set an x label to a new value. It must have the 
# following format:<br>
# setcommand object w col value<br>
# The parameters given to the command are the table objects name, the window
# name of the currently associated cell, the column and the new value.
# If the setxlabelcommand fails, the previous value will be restored.
#}
Classy::Table addoption -setxlabelcommand {setXlabelCommand Command {}} {}
...
#doc {Table options -setylabelcommand} option {-setylabelcommand setylabelCommand Command} descr {
# gives the command that will be called to set an y label to a new value. It must have the 
# following format:<br>
# setcommand object w col value<br>
# The parameters given to the command are the table objects name, the window
# name of the currently associated cell, the column and the new value.
# If the setylabelcommand fails, the previous value will be restored.
#}
Classy::Table addoption -setylabelcommand {setylabelCommand Command {}} {}
...
#} descr {
# query or change the current width of the given column. The size of the label column
# can be changed by giving 'label' as column
#}
...
#} descr {
# query or change the current height of the given row. The size of the label row
# can be changed by giving 'label' as column
#}
...
Classy::Table method _setxl {w args} {
	private $object options dcol data
	if {"$options(-setxlabelcommand)" == ""} return
	regexp {([0-9]+)$} $w temp col
	switch $options(-type) {
		text {
			set value [$w get 1.0 end]
		}
	...
	set rcol [expr {$col+$dcol(start)}]
	set error [catch {uplevel #0 $options(-setxlabelcommand) $object $object.base.xl$col $rcol [list $value]} res]
	if $error {error $res}
}

Classy::Table method _setyl {w args} {
	private $object options drow data
	if {"$options(-setylabelcommand)" == ""} return
	regexp {([0-9]+)$} $w temp row
	switch $options(-type) {
		text {
			set value [$w get 1.0 end]
		}
	...
	set rrow [expr {$row+$drow(start)}]
	set error [catch {uplevel #0 $options(-setylabelcommand) $object $object.base.yl$row $rrow [list $value]} res]
	if $error {error $res}
}
	...
	if {"$options(-ylabelcommand)" != ""} {
		...
		if {$rownum > 0} {
			for {set row $prevrow} {$row < $drow(num)} {incr row} {
				...
				bindtags $object.base.yl$row [list Classy::Table::ylabel $object.base.yl$col Entry . all]
	...
	if {"$options(-xlabelcommand)" != ""} {
		...
		if {$colnum > 0} {
			for {set col $prevcol} {$col < $dcol(num)} {incr col} {
				...
				bindtags $object.base.xl$col [list Classy::Table::xlabel $object.base.xl$col Entry . all]
